...
def get_locate_warning_msg(error_type, error_rules: List):
    ...
    return f"**It seems that the rule {get_rule_id_str(error_rules)} of the access control policy contain{'s a/an' if len(error_rules)==1 else ''} {error_type}{'s' if len(error_rules)>1 else ''}.**\n\n1. Find the access control rule (i.e., row) you need to edit.\n2. Double click on the cell/cells corresponding to the element you need to edit in the table below and write the correct values.\n3. Make sure that no other errors are present in the updated policy.\n4. Click ***Submit*** to submit the corrected policy."

def get_locate_warning_missing_rule_msg():
    return f"**It seems that one or more access control rules are missing in the generated policy.**\n1. Click on a cell in the last row of the table below.\n2. Add the missing rule with the correct policy components.\n3. Make sure that no other errors are present in the updated policy.\n4. Click ***Submit*** to submit the corrected policy."